...
public struct Configuration {
    ...
    ///
    /// - parameter appID: Optionally provide a `String` for an app id for `Session`s.
    /// Default value is the app's `bundleID`.
    /// - parameter blendShapes: Optionally provide an array of `BlendShapeLocation`s to
    /// track any number of `ARKit`'s provided facial recognition values.
    ///
    public init(appID: String? = nil, blendShapes: [ARFaceAnchor.BlendShapeLocation]? = nil) {
        // Fall way back to using a UUID, if bundleIdentifier happens to be nil.
        // Need to investigate when this may happen.
